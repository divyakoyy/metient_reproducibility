...
def predict_vertex_labeling(machina_sims_data_dir, site, mig_type, seed, out_dir, weights, sample_size, weight_init_primary, perf_stats_fn, mode, solve_polytomies):
    ...
    trees = fnmatch.filter(os.listdir(os.path.join(machina_sims_data_dir, f"{site}_clustered_input")), f"cluster_{mig_type}_seed{seed}_tree*.tsv")
    ...
    for tree_num in range(len(trees)):
        ...
        ref_var_fn = os.path.join(machina_sims_data_dir, f"{site}_clustered_input", f"cluster_{mig_type}_seed{seed}_tree{tree_num}.tsv")
