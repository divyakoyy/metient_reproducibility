...
from projects.metient.metient.api import *
...
predictions_dir = os.path.join('./metient_outputs', f"predictions_{run_name}")
...
start_time = datetime.datetime.now()
# 2. Find the best thetas for each migration type category (split by: {primary-only, met-to-met})

gt_df = pd.read_csv("gt_pattern.csv")
gt_df['site'] = gt_df['site'].astype(str)
gt_df['mig_type'] = gt_df['mig_type'].astype(str)
gt_df['seed'] = gt_df['seed'].astype(str)
print(gt_df.head())
mig_type_to_trees = {}
mig_type_to_pt_weights = {}
for site in sites:
    for mig_type in mig_types:
        site_mig_data_dir = os.path.join(machina_sims_data_dir, site, mig_type)
        seeds = fnmatch.filter(os.listdir(site_mig_data_dir), 'reads_seed*.tsv')
        ...
        for seed in seeds:
            num_trees = get_num_mut_trees(os.path.join(machina_sims_data_dir, f"{site}_mut_trees", f"mut_trees_{mig_type}_seed{seed}.txt"))
            print(gt_df[(gt_df['site']==site)&(gt_df['mig_type']==mig_type)&(gt_df['seed']==seed)]['gt_pattern'])
            ...
            if gt_pattern not in mig_type_to_trees:
                mig_type_to_trees[gt_pattern] = []
                mig_type_to_pt_weights[gt_pattern] = []
            pt_weights = []
            for tree_num in range(num_trees):
                T = data[tree_num][0]
                seeding_fn = os.path.join(predictions_dir, site, mig_type, f"tree{tree_num}_seed{seed}_calibrate.pkl.gz")
                mig_type_to_trees[gt_pattern].append(seeding_fn)
                mig_type_to_pt_weights[gt_pattern].append((T.shape[0]/num_trees))
                #mig_type_to_pt_weights[gt_pattern].append((1.0))
                pt_weights.append((T.shape[0]/num_trees))
            print(site, mig_type, seed, "num_trees", num_trees, "tree size", T.shape[0], "pt weights", pt_weights)

mig_type_to_calibration_time = {m:(datetime.datetime.now() - start_time).total_seconds() for m in mig_type_to_trees}

for m in mig_type_to_trees:
    print(m, "num trees:", len(mig_type_to_trees))
mig_type_to_best_theta = {}
for gt_pattern in mig_type_to_trees:
    start_time = datetime.datetime.now()
    best_theta = eutil.get_max_cross_ent_thetas(mig_type_to_trees[gt_pattern], mig_type_to_pt_weights[gt_pattern])
    print("BEST THETA", best_theta, "mig_type", gt_pattern)
    mig_type_to_best_theta[gt_pattern] = best_theta
    mig_type_to_calibration_time[gt_pattern] += (datetime.datetime.now() - start_time).total_seconds()
...
for site in sites:
    for mig_type in mig_types:
        ...
        for seed in seeds:
            ...
            weights = Weights(mig=[mig_type_to_best_theta[gt_pattern][0]*50], comig=mig_type_to_best_theta[gt_pattern][1]*50, seed_site=[mig_type_to_best_theta[gt_pattern][2]*50],
