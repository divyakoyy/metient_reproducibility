...
* `clustered_input_*`
  ...
  * `cluster_*_seed*.tsv` files in `m5_clustered_input` and `m8_clustered_input`  contains reference and variant read counts for the clustered data. These were generated by running create_conf_intervals_from_machina_sims.sh
* `*_mut_trees` were generated by running MACHINA's `generatemutationtrees` executable on the cluster tsv's mentioned above. See generate_mutation_trees.sh
